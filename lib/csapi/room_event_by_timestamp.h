 ...
 * endpoint](/server-server-api/#get_matrixfederationv1timestamp_to_eventroomid)
 ...
 * to obtain a pagination token to retrieve the events around the returned
 * event.
 *
...
public:
     ...
     */
    explicit GetEventByTimestampJob(const QString& roomId, int ts,
                                    const QString& dir);
     ...
     */
    static QUrl makeRequestUrl(QUrl baseUrl, const QString& roomId, int ts,
                               const QString& dir);
    ...
    {
        return loadFromJson<int>("origin_server_ts"_ls);
    }
};
